...
def vertically_integrate(field, z_coords, preserve_structure=False):
    ...
    integrated_field = jnp.concatenate(
            [jnp.zeros_like(dzs[...,-1])[...,None], integrated_field],
            axis=-1
                                      )
...
def interp_single_cell(field_i, zs_i, zs_i_new):
...
def interp_fields_onto_new_zs(fields, zs, zs_new):
    ...
    interp_fn = vmap(interp_single_cell, in_axes=(1, 0, 0))

    n_fields, ny, nx, nz = fields.shape
    flat_fields = fields.reshape(n_fields, -1, nz)
    flat_zs = zs.reshape(-1, nz)
    flat_zs_new = zs_new.reshape(-1, nz)
